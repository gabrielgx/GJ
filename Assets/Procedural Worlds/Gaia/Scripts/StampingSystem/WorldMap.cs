...
////                    if (terrain != null)
////                    {
////                        m_localHeightmapRes = terrain.terrainData.heightmapResolution;
////                        m_localTileSize = terrain.terrainData.size.x;
////                    }
////                }
...
//            {
        ...
        private void OnWorldMapCreate()
        {
            ...
            if (bounds.size.x > 0)
            {
                GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_worldMaprelativeSize = (float)m_currentWorldCreationSettings.m_tileSize / (float)bounds.size.x;
                GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_worldMapRelativeHeightmapPixels = (((float)bounds.size.x / m_localTileSize) * m_localHeightmapRes / m_localHeightmapRes);
            }
            else
            {
                //no bounds size means no real terrains (yet) - set these parameters according to the current settings/defaults then
                GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_worldMaprelativeSize = (float)m_currentWorldCreationSettings.m_tileSize / (gaiaSettings.m_tilesX * currentDefaults.m_terrainSize);
