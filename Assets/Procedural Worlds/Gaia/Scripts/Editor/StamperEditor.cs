...
namespace Gaia
{
    ...
    public class StamperEditor : PWEditor, IPWEditor
    {
        ...
        void OnEnable()
        {
            ...
            float requiredFitToWorldWidth = 100 * Mathf.Max(GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainTilesX, GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainTilesZ);
...
#region AutoSpawnerList
        ...
        {
            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(m_editorUtils.GetContent("MixHeightImage"), GUILayout.Width(EditorGUIUtility.labelWidth));
            m_stamper.m_settings.m_mixHeightImage = (Texture2D)EditorGUILayout.ObjectField(m_stamper.m_settings.m_mixHeightImage, typeof(Texture2D), false, GUILayout.Height(EditorGUIUtility.singleLineHeight));
            if (GUILayout.Button(m_editorUtils.GetContent("MaskImageOpenStampButton")))
            {
                ImageMaskListEditor.OpenStampBrowser(null,m_stamper);
            }
            GUILayout.EndHorizontal();
            m_editorUtils.InlineHelp("MixHeightImage", showHelp);
            m_stamper.m_settings.m_mixMidPoint = m_editorUtils.Slider("MixMidPoint", m_stamper.m_settings.m_mixMidPoint, 0f, 1f, showHelp);
            m_stamper.m_settings.m_mixHeightStrength = m_editorUtils.Slider("MixHeightStrength", m_stamper.m_settings.m_mixHeightStrength, 0f, 2f, showHelp);
        }
