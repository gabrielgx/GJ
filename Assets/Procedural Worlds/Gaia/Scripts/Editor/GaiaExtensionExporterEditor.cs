...
namespace Gaia
{
    ...
    public class GaiaExtensionExporterEditor : EditorWindow
    {
                            ...
                            if (DragAndDrop.objectReferences.Length > 0)
                            {
                                ...
                                if (DragAndDrop.objectReferences[0].GetType() == typeof(UnityEngine.GameObject))
                                {
                                    ...
                                    if (spawner != null)
                                    {
                                        ...
                                        if (m_resourceList.Count == 0)
                                        {
                                            m_resourceList.Add(spawner.m_settings.m_resources);
                                        }
                                        else
                                        {
                                            for (int resIdx = 0; resIdx < m_resourceList.Count; resIdx++)
                                            {
                                                if (m_resourceList[resIdx].m_resourcesID == spawner.m_settings.m_resources.m_resourcesID)// && m_resourceList[resIdx].name == spawner.m_settings.m_resources.name)
                                                {
                                                    return;
                                                }
                                                else
        ...
        private void SaveGX()
        {
            ...
            Template template;
            string hdrPath = PWCommon2.AssetUtils.GetAssetPath("GaiaXtnHeader", "txt");
            string ftrPath = PWCommon2.AssetUtils.GetAssetPath("GaiaXtnFooter", "txt");
        ...
        //        newTex.m_metallic = srcTex.m_metallic;
        ...
        //    resource = (GaiaResource)PWCommon1.AssetUtils.GetAssetScriptableObject(fileName);
        //    #endif
