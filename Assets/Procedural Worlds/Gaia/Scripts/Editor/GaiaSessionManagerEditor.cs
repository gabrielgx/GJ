...
#region OLD CODE
        ...
        private void DrawSummary(bool helpEnabled)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                GaiaTerrainLoaderManager.Instance.SaveStorageData();

                AssetDatabase.DeleteAsset(oldSessionDataPath);
                AssetDatabase.SaveAssets();
            }
            //if (oldName != m_manager.m_session.name)
            //{
            //    //Session was renamed, refresh the session asset so the name change becomes visible
            //    EditorUtility.SetDirty(m_manager.m_session);
            //    AssetDatabase.SaveAssets();
            //    //need to rename the session data path as well
            //    //AssetDatabase.MoveAsset(oldSessionDataPath, GaiaDirectories.GetSessionSciptableObjectPath(m_manager.m_session));
            //}
            EditorGUILayout.BeginHorizontal();
            //float width = EditorGUILayout.GetControlRect().width;
            //width -= EditorGUIUtility.labelWidth;
            EditorGUILayout.LabelField(m_editorUtils.GetContent("Description"), GUILayout.MaxWidth(EditorGUIUtility.labelWidth - 13));
            ...
            if (GUILayout.Button("Generate Image"))
            {
                ...
                OrthographicBake.BakeTerrain(Terrain.activeTerrain, 2048, 2048, textureFileName, Camera.main.cullingMask);
        ...
        private void DrawOperations(bool helpEnabled)
        {
            ...
            for (int i=0;i<m_manager.m_session.m_operations.Count;i++)
            {
                ...
                GUI.enabled = op.m_isActive; 
                op.m_isFoldedOut = m_editorUtils.Foldout(op.m_isFoldedOut, new GUIContent((i+1).ToString() + " " + op.m_description.ToString()), true);
                GUILayout.EndHorizontal();
                GUI.enabled = currentGUIState;
