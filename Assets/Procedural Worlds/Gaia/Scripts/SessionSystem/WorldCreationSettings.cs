...
namespace Gaia
{
    ...
    public class WorldCreationSettings : ScriptableObject
    {
        ...
        public string CheckSettings(bool isWorldMap = false)
        {
            ...
            //Skip this test for the world map, it might be non-POT depending on the total world size
            if (!isWorldMap)
            {
                if (!Mathf.IsPowerOfTwo(m_tileSize))
                {
                    defStr.AppendFormat("Terrain size must be power of 2! {0} was changed to {1}.\n", m_tileSize, Mathf.ClosestPowerOfTwo(m_tileSize));
                    m_tileSize = Mathf.ClosestPowerOfTwo(m_tileSize);
                }
            }
