...
namespace Gaia
{
    ...
    public class GaiaSessionManager : MonoBehaviour
    {
        ...
        public void GetWorldMinMax(ref float minWorldHeight, ref float maxWorldHeight, bool isWorldmap = false)
        {
            ...
            foreach(TerrainMinMaxHeight tmmh in m_session.m_terrainMinMaxCache.Where(x => x.isWorldmap == isWorldmap && x.recalculate))
            {
                float newMin = float.MaxValue;
                float newMax = float.MinValue;
                if (isWorldmap)
                {
                    GetTerrainMinMax(GaiaTerrainLoaderManager.Instance.WorldMapTerrain, ref newMin, ref newMax);
                }
                else
                {
                    if (dynamicLoaded)
                    {
                        ...
                        if (terrainData != null)
                        {
                            ...
                            GaiaUtils.CallFunctionOnDynamicLoadedTerrains( act, false, new List<string>() { terrainData.name });
                        }
                    }
                    else
                    {
            ...
            //if (GaiaUtils.HasDynamicLoadedTerrains() && !isWorldmap)
            //{
            //    //In case of dynamic loaded terrains we must take into account the height values of the currently unloaded terrains as well
            //    //which are available to us in the min max cache
            //    minWorldHeight = m_session.m_terrainMinMaxCache.Where(x=>x.isWorldmap==false).Select(x=>x.min).Min();
            //    maxWorldHeight = m_session.m_terrainMinMaxCache.Where(x => x.isWorldmap == false).Select(x=>x.max).Max();
            //}
            //else
            //{
...
//            UnityEngine.Random.InitState(System.Environment.TickCount);
        ...
        public IEnumerator ContinueMassStamp()
        {
            for (int i = 0; i < m_massStamperSettingsList.Count; i++)
            {
                ...
                if (ProgressBar.Show(ProgressBarPriority.WorldCreation, "Stamping", "Stamping " + m_massStamperSettingsList[i].m_imageMasks[0].m_imageMaskTexture.name, i, m_massStamperSettingsList.Count, true, true))
                {
