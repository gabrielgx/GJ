...
namespace Gaia
{
    ...
    public class GaiaWorldManager
    {
        ...
        public GaiaWorldManager(Terrain[] terrains, bool isWorldMap = false)
        {
            ...
            //kick out terrains indices that are not world map / regular terrains, depending on we are managing a world map or not
            for (int i = terrains.Length - 1; i >= 0; i--)
            {
                if (isWorldMap != TerrainHelper.IsWorldMapTerrain(terrains[i]))
                {
                    terrains = GaiaUtils.RemoveArrayIndexAt(terrains, i);
                }
            }

            string validWorld = IsValidWorld(terrains, isWorldMap);
            ...
            m_worldBoundsNU.size = new Vector3(m_worldBoundsTU.size.x / terrain.terrainData.heightmapResolution, m_worldBoundsTU.size.y / terrain.terrainData.heightmapScale.y, m_worldBoundsTU.size.z / terrain.terrainData.heightmapResolution);


            //Pick up and calc normal units off last terrain
            if (terrain != null)
            {
                m_TUtoNU = new Vector3(m_worldBoundsNU.size.x / m_worldBoundsTU.size.x, m_worldBoundsNU.size.y / m_worldBoundsTU.size.y, m_worldBoundsNU.size.z / m_worldBoundsTU.size.z);
            ...
            //m_worldBoundsNU.size = new Vector3(m_worldBoundsTU.size.x/ terrain.terrainData.heightmapWidth, m_worldBoundsTU.size.y / terrain.terrainData.heightmapScale.y, m_worldBoundsTU.size.z / terrain.terrainData.heightmapHeight);
        ...
        public string IsValidWorld(Terrain[] terrains, bool isWorldMap)
        {
            ...
            for (int idx = 0; idx < terrains.Length; idx++)
            {
                ...
                //Skip on world map terrains
                if (isWorldMap != TerrainHelper.IsWorldMapTerrain(terrain))
                {
                    continue;
                }
        ...
        /// </summary>
        /// <param name="forceWrite">If true, will aways write, otherwise will only write changes</param>
        public bool SaveToWorld(bool forceWrite = false)
        {

            if (UpdatePhysicalTerrainArray())
            {
                UnityHeightMap hm;
                for (int x = 0; x < m_heightMapTerrainArray.GetLength(0); x++)
                {
        ...
        public bool UpdatePhysicalTerrainArray()
        {
            for (int idx = 0; idx < Terrain.activeTerrains.Length; idx++)
            {
                ...
                Vector3 positionPTI = WUtoPTI(terrain.transform.position);
                if (positionPTI.x >= 0 && m_physicalTerrainArray.GetLength(0) > positionPTI.x && positionPTI.z >= 0 && m_physicalTerrainArray.GetLength(1) > positionPTI.z)
                {
                    m_physicalTerrainArray[(int)positionPTI.x, (int)positionPTI.z] = terrain;
                }
                else
                {
                    Debug.LogWarning("Mismatch of undo data and the currently loaded terrains, your undo could not be restored. Make sure the same terrains are loaded in as when the operation for the undo was performed.");
                    return false;
                }
            }
        ...
        public void ExportGrassmapAsPng(string path)
        {
            ...
            GaiaUtils.CompressToMultiChannelFileImage(detailMaps, path, TextureFormat.RGBA32, false, true, false);
        }
        ...
        public void ExportNormalmapAsPng(string path)
        {
            for (int tileX = 0; tileX < m_physicalTerrainArray.GetLength(0); tileX++)
            {
                for (int tileZ = 0; tileZ < m_physicalTerrainArray.GetLength(1); tileZ++)
                {
                    ...
                    PWCommon2.Utils.WriteAllBytes(spath, pngBytes);
            ...
            for (int tileX = 0; tileX < m_physicalTerrainArray.GetLength(0); tileX++)
            {
                for (int tileZ = 0; tileZ < m_physicalTerrainArray.GetLength(1); tileZ++)
                {
                    ...
                    if (terrain != null)
                    {
                        ...
                        GaiaUtils.CompressToMultiChannelFileImage(nrmMap, path + "_" + tileX + "_" + tileZ, TextureFormat.RGBA32, false, true, false);
#if UNITY_EDITOR
                        AssetDatabase.Refresh();
            ...
            for (float x = minX; x < maxX; x += 1f)
            {
                ...
                for (float z = minZ; z < maxZ; z += 1f)
                {
                    ...
                    if (InBoundsTU(checkPos))
                    {
                        ...
                        {
                            strength = PWCommon2.Utils.Math_Distance(x, z, positionTU.x, positionTU.z) / maskSizeTU;
                            if (strength <= 1f)
                            {
