...
inline void WindMovement_float ( in float3 i_vertexPos, in float i_windSpeed, in float i_windGust, in float3 i_windDirection, out float3 o_vertexPos )
{
	...
	CombineLocalGlobal ( i_windSpeed, _PW_Global_WindSpeed, windSpeed );
	CombineLocalGlobal (i_windGust, _PW_Global_WindGustDistance, windGust );

	CombineLocalGlobal ( i_windDirection.x, _PW_Global_WindDirection.x, windX );
	CombineLocalGlobal ( i_windDirection.y, _PW_Global_WindDirection.y, windY );
	CombineLocalGlobal ( i_windDirection.z, _PW_Global_WindDirection.z, windZ );
	...
	float height = ( i_vertexPos.y / 2.0 + 0.5 );

	float speed = _Time.x * windSpeed;

	float moveX = sin ( speed + i_vertexPos.x * windGust ) * height * windX;
	float moveY = sin ( speed + i_vertexPos.y * windGust ) * windY;
	float moveZ = sin ( speed + i_vertexPos.z * windGust ) * height * windZ;

	// for when we have vertex movement color
//	#ifdef _PW_GD_WINDVERTEX_ON
//	moveX *= i_vertexStrength.x;
//	moveY *= i_vertexStrength.y;
//	moveZ *= i_vertexStrength.z;
//	#endif


	o_vertexPos = i_vertexPos + float3(moveX, moveY, moveZ);
#else
