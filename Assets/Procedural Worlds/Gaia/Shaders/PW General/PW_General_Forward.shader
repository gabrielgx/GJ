Shader "PWS/PW_General_Forward"
{
    Properties
    {
		...
		[Toggle(_ALPHATEST_ON)]_AlphaTest("Cutout", Float) 						= 0
		...
		_AOVertexMask ("Vertex Mask", Range(0,1))   				   			= 1.0
   		...
   		_PW_WorldMapToggle ( "Toggle", float ) 									= 0
		...
		[KeywordEnum(Off,On)] _PW_Cover("Cover", Float) 						= 0
		...
		_PW_WindToggle("Wind", Float)  											= 0
		_PW_WindVertex("Use Vertex Color", Float)  								= 0
		_PW_WindDirection ( "Direction Vector", Vector )						= ( 0.01, 0.01, 0.01, 0.0 )
   		_PW_WindSpeed ( "Speed", float ) 										= 0.57
   		_PW_WindGustDistance ( "Gust Spacing", float ) 							= 1.4
    ...
    SubShader
    {
		...
		#pragma shader_feature_local _PW_SF_GLOBAL_CONTROL_ON
		...
		void vert ( inout appdata_full v, out Input data )
		{
			...
			WindMovement_float ( v.vertex.xyz, _PW_WindSpeed, _PW_WindGustDistance, _PW_WindDirection, v.vertex.xyz );
