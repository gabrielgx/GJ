    ...
    SubShader {
        ...
        CGINCLUDE
			...
			float GetFinalHeight(v2f i)
			{
				...
				/*float transformheight = UnpackHeightmap(tex2D(_TransformHeightTex,float2(brushShape*2,0)));

				

				float globalMaskValue = 1.0f;
				float localMaskValue = 1.0f;

				if(_ImageMaskIsGlobal)
				{
					globalMaskValue *= imageMask;
				}
				else
				{
					localMaskValue *= imageMask;
				}

				if(_DistanceMaskIsGlobal)
				{
					globalMaskValue *= distanceMask;
				}
				else
				{
					localMaskValue *= distanceMask;
				}*/
				...
				{
					//globalMaskValue = 0.5f;
					float finalResult = _yPos + finalBrushLevel;
					//return clamp(finalResult + height * (1.0f - finalResult), 0, 1.0f);*/

					//if (finalResult < height)
					//{
						//return height + (finalResult - height) * globalMaskValue;
					//}
					//else
					//{
					//return lerp(height, finalResult * globalMaskValue, globalMaskValue);
					//}

					//return lerp(height, finalResult, globalMaskValue);
					return clamp(((_yPos + finalBrushLevel) * globalMaskValue) + height * (1.0f - globalMaskValue), 0, 1.0f);
					//return clamp(((_yPos + finalBrushLevel) * globalMaskValue) + 0.1f, 0, 1.0f);
					//return clamp((_yPos  + finalBrushLevel) * globalMaskValue, 0, 1.0f);
				}
				else
				{
					return height;
