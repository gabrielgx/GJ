    ...
    SubShader {
	   ...
	   /// <returns></returns>
		...
		//Gets the "second color" that represents the fitness from the issue
		float GetCol2(v2f i)
		{
			float col2 = 0;
			switch (_FilterMode)
			{
			case 0:
				col2 = UnpackHeightmap(tex2D(_ImageMaskTex, i.pcUV));
				break;
			case 1:
				float4 sampledColor = tex2D(_ImageMaskTex, i.pcUV);
				const float epsilon = 0.00001;
				float difference = RGBDifference(sampledColor, _Color, epsilon);
				...
				break;
			case 2:
				col2 = tex2D(_ImageMaskTex, i.pcUV).r;
				break;
			case 3:
				col2 = tex2D(_ImageMaskTex, i.pcUV).g;
				break;
			case 4:
				col2 = tex2D(_ImageMaskTex, i.pcUV).b;
				break;
			case 5:
				col2 = tex2D(_ImageMaskTex, i.pcUV).a;
				break;
			}
			return col2;
		}
